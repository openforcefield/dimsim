...
def test_find_exclusions_simple():
    ...
    exclusions = dimsim.utils.find_exclusions(molecule.to_topology())
    assert exclusions == {
        (0, 1): "scale_12",
        (0, 2): "scale_13",
        (0, 3): "scale_14",
...
def test_find_exclusions_dimer():
    ...
    exclusions = dimsim.utils.find_exclusions(topology)
    assert exclusions == {
        (0, 1): "scale_12",
        (0, 2): "scale_13",
        (1, 2): "scale_12",
...
def test_find_exclusions_v_sites():
    ...
    v_sites = dimsim.VSiteMap(
        keys=v_site_keys,
        key_to_idx={v_site_keys[0]: 4, v_site_keys[1]: 5},
        parameter_idxs=torch.zeros((2, 1)),
    )

    exclusions = dimsim.utils.find_exclusions(molecule.to_topology(), v_sites)
    assert exclusions == {
        (0, 1): "scale_12",
        (0, 2): "scale_13",
        (0, 3): "scale_14",
...
def test_ones_like():
    ...
    tensor = dimsim.utils.ones_like(expected_size, other)
...
def test_zeros_like():
    ...
    other = torch.tensor([1, 2, 3], dtype=expected_type, device="cpu")
    tensor = dimsim.utils.zeros_like(expected_size, other)
...
def test_tensor_like():
    ...
    tensor = dimsim.utils.tensor_like(expected_data, other)
...
def test_arange_like():
    ...
    other = torch.tensor([], dtype=expected_type, device="cpu")
    tensor = dimsim.utils.arange_like(4, other)
...
def test_to_upper_tri_idx(n):
    ...
    idxs = dimsim.utils.to_upper_tri_idx(i, j, n)
...
def test_geometric_mean():
    ...
    assert torch.autograd.gradcheck(
        dimsim.utils.geometric_mean,
        (a, b),
        check_backward_ad=True,
        check_forward_ad=True,
    )

    assert torch.isclose(dimsim.utils.geometric_mean(a, b), torch.tensor(6.0).double())
...
@pytest.mark.parametrize(
    ...
    [
        (0.0, 0.0, 0.0, 0.0),
        (3.0, 0.0, 0.0, 3.0 / (2.0 * dimsim.utils.EPSILON)),
        (0.0, 4.0, 4.0 / (2.0 * dimsim.utils.EPSILON), 0.0),
    ],
)
def test_geometric_mean_zero(a, b, expected_grad_a, expected_grad_b):
    ...
    v = dimsim.utils.geometric_mean(a, b)
    v.backward()
