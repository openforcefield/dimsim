...
from dimsim.mm._reporters import _encoder


def _mock_potential(type_, parameters, attributes) -> dimsim.TensorPotential:
    return dimsim.TensorPotential(
        type_,
        f"{type_}-fn",
        parameters,
...
def test_pack_unpack_force_field(mocker):
    ...
    force_field = dimsim.TensorForceField(
        ...
        dimsim.TensorVSites(v_site_keys, v_site_weights, v_site_parameters),
    )
...
def test_compute_mass():
    water = dimsim._tests.utils.topology_from_smiles("O")
    methane = dimsim._tests.utils.topology_from_smiles("C")

    system = dimsim.TensorSystem([water, methane], [2, 3], True)
...
def test_compute_observables(tmp_path, mock_argon_tensors, mock_argon_params):
    ...
    tensor_system = dimsim.TensorSystem([tensor_top], [2], False)
...
def test_compute_ensemble_averages(mocker, tmp_path, mock_argon_tensors):
    ...
    mock_compute_observables = mocker.patch(
        "dimsim.mm._ops._compute_observables",
        autospec=True,
