...
def test_topology_to_rdkit():
    ...
    topology = dimsim.TensorTopology(
...
def test_topology_to_xyz(mocker):
    ...
    mocker.patch("dimsim.mm._utils.topology_to_rdkit", return_value=mock_molecule)
...
def test_generate_system_coords():
    coords, box_vectors = generate_system_coords(
        dimsim.TensorSystem(
            [
                dimsim._tests.utils.topology_from_smiles("O"),
                dimsim._tests.utils.topology_from_smiles("CO"),
            ],
            [1, 2],
            True,
        ),
        None,
        dimsim.mm.GenerateCoordsConfig(),
    )
...
def test_simulate(mocker, mock_argon_tensors):
    ...
    spied_energy_minimize = mocker.spy(dimsim.mm._mm, "_energy_minimize")
    spied_run_simulation = mocker.spy(dimsim.mm._mm, "_run_simulation")
