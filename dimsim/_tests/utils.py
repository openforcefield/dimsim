...
class LJParam(typing.NamedTuple):
    eps: float
    sig: float
...
def mock_convert_fn_with_deps(
    ...
    dependencies: dict[str, tuple[dimsim.TensorPotential, list[dimsim.NonbondedParameterMap]]],
) -> tuple[dimsim.TensorPotential, list[dimsim.NonbondedParameterMap]]:
    raise NotImplementedError()


def convert_lj_to_dexp(potential: dimsim.TensorPotential):
    potential.fn = dimsim.EnergyFn.VDW_DEXP
...
def topology_from_smiles(smiles: str) -> dimsim.TensorTopology:
    ...
    return dimsim.TensorTopology(
        atomic_nums=torch.tensor([atom.GetAtomicNum() for atom in mol.GetAtoms()]),
        formal_charges=torch.tensor([atom.GetFormalCharge() for atom in mol.GetAtoms()]),
        bond_idxs=torch.tensor([[bond.GetBeginAtomIdx(), bond.GetEndAtomIdx()] for bond in mol.GetBonds()]),
        bond_orders=torch.tensor([int(bond.GetBondTypeAsDouble()) for bond in mol.GetBonds()]),
        parameters={},
        v_sites=None,
        constraints=None,
    )
...
def system_from_smiles(
...
) -> tuple[dimsim.TensorSystem, dimsim.TensorForceField]:
    ...
    """
    force_field = force_field if force_field is not None else openff.toolkit.ForceField("openff-2.0.0.offxml")
    ...
    return dimsim.TensorSystem(tensor_tops, n_copies, is_periodic=True), tensor_ff


def _parameter_key_to_idx(potential: dimsim.TensorPotential, key: str):
    return next(iter(i for i, k in enumerate(potential.parameter_keys) if k.id == key))


def system_with_exceptions() -> tuple[dimsim.TensorSystem, dimsim.TensorPotential, dict[str, LJParam]]:
    ...
    lj_handler.add_parameter({"smirks": "[O:1]", "epsilon": eps_o * kcal, "sigma": sig_o * ang})
    lj_handler.add_parameter({"smirks": "[H:1]", "epsilon": eps_h * kcal, "sigma": sig_h * ang})
    lj_handler.add_parameter({"smirks": "[Ar:1]", "epsilon": eps_a * kcal, "sigma": sig_a * ang})

    system, tensor_ff = dimsim._tests.utils.system_from_smiles(["O", "[Ar]"], [1, 1], ff)
    ...
    for top in system.topologies:
        assignment_matrix = dimsim.utils.zeros_like(
...
def add_explicit_lb_exceptions(potential: dimsim.TensorPotential, system: dimsim.TensorSystem):
    ...
    eps_ij, sig_ij = dimsim.potentials.nonbonded.lorentz_berthelot(eps_i, eps_j, sig_i, sig_j)
    ...
    for i in range(len(eps_ij)):
        potential.parameter_keys.append(
            openff.interchange.models.PotentialKey(id=f"exception-{i}", associated_handler="vdW")
        )

    potential.exceptions = {
        (int(idx_i), int(idx_j)): i + n_params for i, (idx_i, idx_j) in enumerate(zip(idxs_i, idxs_j, strict=True))
    }

    for top in system.topologies:
        parameter_map = top.parameters["vdW"]

        padding = dimsim.utils.zeros_like(
