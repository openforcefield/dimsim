...
import dimsim
from dimsim.converters.openff._openff import (
    _CONVERTERS,
    _convert_topology,
    _Converter,
...
def test_convert_handler(ethanol, ethanol_interchange, mocker):
    ...
    mock_convert = mocker.patch(
        "dimsim._tests.utils.mock_convert_fn_with_deps",
        autospec=True,
...
def test_convert_topology(formaldehyde, mocker):
    ...
    v_sites = dimsim.VSiteMap([], {}, torch.tensor([]))
    constraints = dimsim.TensorConstraints(torch.tensor([1, 2]), torch.tensor([3.0]))
