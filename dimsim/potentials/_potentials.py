...
def potential_energy_fn(handler_type: str, energy_expression: str):
    ...
    def _potential_function_inner(func):
        if (handler_type, energy_expression) in _POTENTIAL_ENERGY_FUNCTIONS:
            raise KeyError(
                f"A potential energy function is already defined for handler={handler_type} fn={energy_expression}."
            )
...
def broadcast_parameters(system: dimsim.TensorSystem, potential: dimsim.TensorPotential) -> torch.Tensor:
    ...
    return torch.zeros((0, len(potential.parameter_cols))) if len(parameters) == 0 else torch.vstack(parameters)
...
def broadcast_exceptions(
    ...
    for topology, n_copies in zip(system.topologies, system.n_copies, strict=True):
        ...
        assigned_idxs = torch.broadcast_to(assigned_idxs[None, :], (n_copies, n_particles)).flatten()

        parameter_idxs.append(assigned_idxs)

    if len(parameter_idxs) == 0:
        return torch.zeros((0,), dtype=torch.int64), torch.zeros((0, 0, len(potential.parameter_cols)))

    parameter_idxs = torch.concat(parameter_idxs)
    parameter_idxs_a = parameter_idxs[idxs_a]
    parameter_idxs_b = parameter_idxs[idxs_b]

    if len({(min(i, j), max(i, j)) for i, j in potential.exceptions}) != len(potential.exceptions):
        raise NotImplementedError("cannot define different exceptions for i-j and j-i")

    exception_lookup = torch.full((len(potential.parameters), len(potential.parameters)), -1)
...
def broadcast_idxs(system: dimsim.TensorSystem, potential: dimsim.TensorPotential) -> torch.Tensor:
    ...
    return torch.zeros((0, 0)) if len(per_topology_idxs) == 0 else torch.vstack(per_topology_idxs)
