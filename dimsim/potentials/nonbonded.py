...
_COULOMB_PRE_FACTOR = (_UNIT.avogadro_constant / (4.0 * _UNIT.pi * _UNIT.eps0)).m_as(_COULOMB_PRE_FACTOR_UNITS)
...
def _compute_switch_fn(
    ...
    switch_fn = torch.where(pairwise.distances < switch_width, torch.tensor(1.0), switch_fn)
    switch_fn = torch.where(pairwise.distances > pairwise.cutoff, torch.tensor(0.0), switch_fn)
...
def _integrate_lj_switch(
    ...
    integral = -(b**3) * sig**6 * (coeff_01 * r_pow[:6] + coeff_11 * r_pow[6:]).sum(dim=-1) / 252
...
def _integrate_dexp_switch(
    ...
    mat_coeff = smee.utils.tensor_like(
        [
            ...
            [-6,    -6,    -3,    -1,    0,    0,   0,  0],
            [-24,   -24,   -12,   -4,   -1,    0,   0,  0],
            [-120,  -120,  -60,   -20,  -5,   -1,   0,  0],
            [-720,  -720,  -360,  -120, -30,  -6,  -1,  0],
...
def _compute_dexp_lrc(
    ...
    integral = -(
        a_rep * torch.exp(-b_rep * rc) * (-2 / b_rep**3 - 2 * rc / b_rep**2 - rc**2 / b_rep)
        + a_att * torch.exp(-b_att * rc) * (2 / b_att**3 + 2 * rc / b_att**2 + rc**2 / b_att)
    )
...
def _compute_pme_exclusions(system: smee.TensorSystem, potential: smee.TensorPotential) -> torch.Tensor:
    ...
    """
    exclusion_templates = [[[] for _ in range(topology.n_particles)] for topology in system.topologies]
    max_exclusions = 0
    ...
    for exclusions, topology, n_copies in zip(exclusion_templates, system.topologies, system.n_copies, strict=True):
        ...
        exclusions = torch.broadcast_to(exclusions, (n_copies, len(exclusions), max_exclusions))
        exclusions = torch.where(exclusions >= 0, exclusions + exclusion_offset[:, None, None], exclusions)

        exclusions_per_type.append(exclusions.reshape(-1, max_exclusions))

    return (
        torch.zeros((0, 0), dtype=torch.int32) if len(exclusions_per_type) == 0 else torch.vstack(exclusions_per_type)
    )
...
def _compute_coulomb_energy_periodic(
    ...
    pme = NNPOps.pme.PME(grid_x, grid_y, grid_z, _PME_ORDER, alpha, _COULOMB_PRE_FACTOR, exceptions)
    ...
    exclusion_distances = (conformer[exclusion_idxs[:, 0], :] - conformer[exclusion_idxs[:, 1], :]).norm(dim=-1)
...
def compute_coulomb_energy(
    ...
    if system.is_periodic:
        return _compute_coulomb_energy_periodic(system, conformer, box_vectors, potential, pairwise)
    else:
