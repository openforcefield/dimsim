...
import dimsim.mm._config
import dimsim.mm._reporters
import dimsim.mm._utils

_LOGGER = logging.getLogger("dimsim.mm")
...
def _apply_hmr(
    ...
    Args:
        ...
        system_dimsim: The dimsim system that defines the system topology.
...
def generate_system_coords(
    system: dimsim.TensorSystem,
    force_field: dimsim.TensorForceField | None,
    config: typing.Optional["dimsim.mm.GenerateCoordsConfig"] = None,
    ...
    config = config if config is not None else dimsim.mm.GenerateCoordsConfig()
...
def _energy_minimize(
    ...
    config: "dimsim.mm.MinimizationConfig",
) -> openmm.State:
...
def _run_simulation(
    ...
    state: openmm.State | tuple[openmm.unit.Quantity, openmm.unit.Quantity],
    platform: openmm.Platform,
    config: "dimsim.mm.SimulationConfig",
...
def simulate(
    ...
    """Simulate a dimsim system of molecules or topology.
    ...
    omm_system = dimsim.converters.convert_to_openmm_system(force_field, system)
    omm_topology = dimsim.converters.convert_to_openmm_topology(system)
    ...
    for i, config in enumerate(equilibrate_configs):
        ...
        if isinstance(config, dimsim.mm.MinimizationConfig):
