...
def _pack_force_field(
    ...
    """Pack a dimsim force field into a tuple that can be consumed by a custom PyTorch op.
...
def _unpack_force_field(
    ...
    has_v_sites: bool,
    force_field: dimsim.TensorForceField,
) -> dimsim.TensorForceField:
    """Unpack a dimsim force field from its packed tensor and lookup representation.
...
def _compute_mass(system: dimsim.TensorSystem) -> float:
...
def _compute_observables(
    ...
    for coords, box_vectors, _, kinetic in dimsim.mm._reporters.unpack_frames(frames_file):
        ...
        with torch.enable_grad():
            potential = dimsim.compute_energy(system, force_field, coords, box_vectors)
        ...
        if len(needs_grad) > 0:
            du_d_theta_subset = torch.autograd.grad(
                ...
                [dimsim.utils.ones_like(1, potential)],
                retain_graph=False,
                allow_unused=True,
            )
    ...
    reduced_potentials = dimsim.utils.tensor_like(reduced_potentials, theta[0])
...
class _ReweightAverageOp(torch.autograd.Function):
    ...
    def forward(ctx, kwargs: _ReweightAverageKwargs, *theta: torch.Tensor):
        ...
        with kwargs["frames_path"].open("rb") as file:
            reduced_pot_0 = dimsim.utils.tensor_like(
                [v for _, _, v, _ in dimsim.mm._reporters.unpack_frames(file)],
                reduced_pot,
            )
