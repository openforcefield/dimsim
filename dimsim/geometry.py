...
def compute_bond_vectors(conformer: torch.Tensor, atom_indices: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    if len(atom_indices) == 0:
        return (
            dimsim.utils.tensor_like([], other=conformer),
            dimsim.utils.tensor_like([], other=conformer),
        )
...
def compute_angles(conformer: torch.Tensor, atom_indices: torch.Tensor) -> torch.Tensor:
    ...
    if len(atom_indices) == 0:
        return dimsim.utils.tensor_like([], other=conformer)
...
def _build_v_site_coord_frames(
    ...
    for key, weight in zip(v_sites.keys, weights, strict=True):
        ...
        z_norm_clamped = torch.where(
            torch.isclose(z_norm, dimsim.utils.tensor_like(0.0, other=z_norm)),
            dimsim.utils.tensor_like(1.0, other=z_norm),
            z_norm,
        )
        ...
        y_norm_clamped = torch.where(
            torch.isclose(y_norm, dimsim.utils.tensor_like(0.0, other=y_norm)),
            dimsim.utils.tensor_like(1.0, other=y_norm),
            y_norm,
        )
...
def compute_v_site_coords(
    ...
    if len(v_sites.parameter_idxs) > 0:
        ...
        v_site_coords = dimsim.utils.zeros_like((len(conformer), 0, 3), other=conformer)
