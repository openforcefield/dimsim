...
def _create_nonbonded_force(
    ...
    cutoff_idx = potential.attribute_cols.index(dimsim.CUTOFF_ATTRIBUTE)
    switch_idx = (
        None
        if dimsim.SWITCH_ATTRIBUTE not in potential.attribute_cols
        else potential.attribute_cols.index(dimsim.SWITCH_ATTRIBUTE)
    )
...
def _detect_parameters(
    potential: dimsim.TensorPotential, energy_fn: str, mixing_fn: dict[str, str]
