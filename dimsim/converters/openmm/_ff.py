"""Convert dimsim parameters to OpenMM ForceField XML files."""
...
def ffxml_converter(potential_type: str, energy_expression: str):
    ...
    The decorated function should take a `dimsim.TensorPotential`, and
    the associated `dimsim.ParameterMap` and list of atom types, and return a
...
def _get_param_col(pot: dimsim.TensorPotential, param_name: str) -> tuple[int, float]:
...
def _to_residue(
    top: dimsim.TensorTopology,
    ff: dimsim.TensorForceField,
...
def default_valence_converter(
    pot: dimsim.TensorPotential,
    param_map: dimsim.ValenceParameterMap,
    atom_types: list[str],
...
@ffxml_converter(dimsim.PotentialType.VDW, dimsim.EnergyFn.VDW_LJ)
def convert_lj_force(
    pot: dimsim.TensorPotential,
    param_map: dimsim.NonbondedParameterMap,
    types: list[str],
) -> ElementTree.Element:
...
@ffxml_converter(dimsim.PotentialType.ELECTROSTATICS, dimsim.EnergyFn.COULOMB)
...
def _convert_to_openmm_ffxml(
    ...
    omm_top = dimsim.converters.convert_to_openmm_topology(top)
    ...
    for atom in omm_top.atoms():
        ...
        atom_name_to_type[atom.name] = f"dimsim-{short_uuid}-{atom_type}"
...
def convert_to_openmm_ffxml(
    ...
    """Convert a dimsim force field and system to OpenMM force field XML
    representations.
