    ...
    for topology, n_copies in zip(system.topologies, system.n_copies, strict=True):
        for _ in range(n_copies):
            ...
            for key, parameter_idx in zip(topology.v_sites.keys, topology.v_sites.parameter_idxs, strict=True):
                ...
                local_frame_coords = dimsim.geometry.polar_to_cartesian_coords(
...
def _apply_constraints(omm_system: openmm.System, system: dimsim.TensorSystem):
    idx_offset = 0
...
def convert_to_openmm_force(potential: dimsim.TensorPotential, system: dimsim.TensorSystem) -> list[openmm.Force]:
    ...
    importlib.import_module("dimsim.converters.openmm.nonbonded")
    importlib.import_module("dimsim.converters.openmm.valence")
...
def convert_to_openmm_system(
    ...
    """Convert a ``dimsim`` force field and system / topology into an OpenMM system.
...
def convert_to_openmm_topology(
    ...
    """Convert a ``dimsim`` system to an OpenMM topology.
