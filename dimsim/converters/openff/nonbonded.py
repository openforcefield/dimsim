...
def convert_nonbonded_handlers(
    ...
    for handler, topology, v_site_map in zip(handlers, topologies, v_site_maps, strict=True):
        ...
        if has_exclusions:
            exclusion_to_scale = dimsim.utils.find_exclusions(topology, v_site_map)
        ...
        parameter_map = dimsim.NonbondedParameterMap(
...
def convert_dexp(
    ...
    (
        potential,
        parameter_maps,
    ) = dimsim.converters.openff.nonbonded.convert_nonbonded_handlers(
        handlers,
        "DoubleExponential",
        topologies,
        v_site_maps,
    ...
    potential.type = dimsim.PotentialType.VDW
...
def _convert_charge_increment_handlers(
    ...
    potential_bci = dimsim.converters.openff._openff._handlers_to_potential(
