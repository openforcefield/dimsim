...
) -> tuple[dimsim.TensorVSites, list[dimsim.VSiteMap | None]]:
    ...
    parameters = [
        [
            _get_value(
                parameters_by_key[parameter_key],
                column,
                dimsim.TensorVSites.default_units(),
...
) -> list[dimsim.TensorConstraints | None]:
    handler_types = {handler.type for handler in handlers}
...
def _convert_topology(
    ...
    parameters: dict[str, dimsim.ParameterMap],
    v_sites: dimsim.VSiteMap | None,
    constraints: dimsim.TensorConstraints | None,
) -> dimsim.TensorTopology:
...
def convert_interchange(
...
) -> tuple[dimsim.TensorForceField, list[dimsim.TensorTopology]]:
    ...
    tensor_force_field = dimsim.TensorForceField(potentials, v_sites)
